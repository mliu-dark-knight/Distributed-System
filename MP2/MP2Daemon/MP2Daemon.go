...
const check_failure_interval_ms = 400
const send_update_interval_ms = 600
...
func send_join_and_get_current_membership_list() {
	...
	for _, introducer := range introducers {
		join_message := Message{"join", []MembershipTableEntry{members[myEntryIndex].membership_info}}
...
func leave() {
	...
	leave_message := Message{"leave", []MembershipTableEntry{members[myEntryIndex].membership_info}}
...
func synchronize_propagate_membership() {
	for range time.NewTicker(time.Duration(synchronize_interval_ms) * time.Millisecond).C {
		if active {
			...
			change_message := Message{"change", full_list}
...
func handle_member_join(sender_ip string, change_list []MembershipTableEntry) {
	...
	join_reply_message := Message{"change", current_members}
 ...
 */
func send_message_to_neighbors(targets []MembershipInfoEntry, message Message)  {
...
func send_heartbeat()  {
	for range time.NewTicker(time.Duration(heartbeat_interval_ms) * time.Millisecond).C {
		if active {
			...
			if len(targets) > 0 {
				send_message_to_neighbors(targets, Message{"hello",
...
func send_updates() {
	for range time.NewTicker(time.Duration(send_update_interval_ms) * time.Millisecond).C {
		if active {
			...
			if need_update {
				change_message := Message{"change", change_list}
				send_message_to_neighbors(gather_neighbors(), change_message)
			}
...
func interactive(JSONConfigFilefd *os.File) {
	for true {
		...
		case "list":
			...
			//List the current incarnation's ID - which is the IP and the InstanceTimestamp of this instance of the program
		case "id":
			if active {
				fmt.Printf("My ID is {IP: %s, InstanceTimestamp: %s}\n", members[myEntryIndex].membership_info.Ip,
...
func main() {
	...
	JSONConfigFilePath := flag.String("json_config_path", os.Getenv("GOPATH")+"/src/MP2/MP2Daemon/config.json",
	...
	LogFilefd, OSLogOpenErr := os.OpenFile(os.Getenv("GOPATH")+"/src/MP2/MP2Daemon/daemon.log", os.O_RDWR|
